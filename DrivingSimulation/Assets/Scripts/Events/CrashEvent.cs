...
public class CrashEvent
{
    ...
    private GameObject carOne, carTwo, despawnDetect;
    ...
    public void StartCrashEvent(){
        ...
        EventManager.Instance.StartChildCoroutine(playerVehicle.Pathing(TrafficManager.Instance.GetClosestForwardWaypoint(
                playerVehicle.gameObject, playerVehicle.forwardPoint.position), GleyTrafficSystem.Manager.GetClosestWaypoint(crashEventPosition), OnAtEventPosition));
        
        // intialize player detection object for despawning other traffic vehicles
        despawnDetect = new GameObject("DespawnDetect");
        // despawnDetect.transform.position = despawnDetectPosition;
    ...
    public void OnCrashEventEnd(){
        GameObject.Destroy(carOne);
        GameObject.Destroy(carTwo);
    }
}
